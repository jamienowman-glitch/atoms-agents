...
export const ToolPop: React.FC<ToolPopProps> = ({
    ...
    return (
        <div className="w-full max-w-xl mx-auto rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-lg p-4 space-y-3">
            <div className="text-sm font-semibold text-neutral-900 dark:text-neutral-50">ToolPop</div>
            <SliderRow label="Width" value={width} min={64} max={1920} step={4} onChange={(v) => onSliderChange('width', v)} />
            <SliderRow label="Height" value={height} min={64} max={1080} step={4} onChange={(v) => onSliderChange('height', v)} />
            <SliderRow label="Opacity" value={opacity} min={0} max={100} step={1} onChange={(v) => onSliderChange('opacity', v)} />
        </div>
    );
};
