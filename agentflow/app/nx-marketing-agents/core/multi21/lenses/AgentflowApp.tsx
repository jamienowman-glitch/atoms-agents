// =============================================================================
// LensGraphView.tsx
// Ported & Adapted for Multi21 Graph Lens
// =============================================================================
...
const CanvasApi = { getSSEUrl: (id: string) => '' }; // Mock
const TokenApi = { getCatalog: async (id: string) => [] }; // Mock
...
export const AgentflowApp: React.FC<LensGraphViewProps> = ({ deviceMode = 'desktop', canvasId = 'default', onNodeSelect }) => {
    ...
    // Chat Rail State
    const [chatMode, setChatMode] = useState<'nano' | 'micro' | 'standard' | 'full'>('nano');
    ...
    useEffect(() => {
        ...
        // SSE mock or implementation would go here...
    }, [canvasId]);
    ...
    const deleteNode = async (id: string) => {
        ...
        // await LensRegistry.logOp...
    };

    // --- Global Event Listeners ---
    useEffect(() => {
        if (deviceMode === 'mobile') return; // Disable interactions on mobile
    ...
    const handleNodePointerDown = (e: React.PointerEvent, nodeId: string) => {
        if (deviceMode === 'mobile') return; // Mobile click handled by onClick
        e.stopPropagation();
    ...
    const getPortPos = (node: GraphNode, type: 'in' | 'out') => {
        let w = 120; let h = 60; // Generic default
    ...
    const renderNodeInstance = (node: GraphNode) => {
        const props = { ...node.data, data: node.data }; // Pass data as data prop too
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', touchAction: 'none' }}>
            ...
            <div style={{ padding: '8px 20px', borderBottom: '1px solid #eee', background: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <h3 style={{ margin: 0, fontWeight: 900, textTransform: 'uppercase', fontSize: '14px' }}>Agentflow App</h3>
                    <span className="text-xs text-neutral-400 bg-neutral-100 px-2 py-1 rounded">{deviceMode.toUpperCase()}</span>
                </div>
                {/* Add simple mode switch or info */}
            </div>
            ...
            {/* Chat Rail Shell */}
            <ChatRailShell mode={chatMode} onModeChange={setChatMode} />

            {/* Zoom Controls (Desktop) */}
            {deviceMode === 'desktop' && (
                <div style={{ position: 'absolute', bottom: 100, right: 20, display: 'flex', flexDirection: 'column', gap: 5 }}>
