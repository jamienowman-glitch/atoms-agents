...
interface BottomControlsPanelProps {
    ...
    isVisible?: boolean;
    chatMode?: 'nano' | 'micro' | 'standard' | 'full'; // NEW PROP
}

export const BottomControlsPanel: React.FC<BottomControlsPanelProps> = ({ settingsContent, isVisible = false, chatMode = 'nano' }) => {
    ...
    // Auto-Expand Logic (User Request Phase 4)
    // "have it expanded... not in its own nano mode"
    // When tools become visible, if it was collapsed, force it to 'compact' so the buttons are seen
    // If user explicitly Collapsed it via UI, we typically respect that, BUT 
    // the user explicitly asked for "click tool -> show expanded".
    useEffect(() => {
        if (isVisible) {
            // Force expand to compact if currently collapsed
            // Or restore last non collapsed
    ...
    // 5. Visibility Logic (Toaster Animation) - FROM PROPS
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 flex flex-col gap-3">
                            ...
                            <div className="flex flex-col w-full gap-4">
                                {/* Row 1: Controls (Settings + Magnifiers) */}
                                ...
                                {/* Row 2: Sliders / Settings Content */}
                                <div className="w-full">
                                    {activeSlider === 'settings' ? (
                                        <div className="text-xs animate-fadeIn p-2 bg-neutral-50 dark:bg-neutral-900 rounded-md">
                                            ...
                                            <div className="flex flex-col gap-3 animate-fadeIn p-1">
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex justify-between items-center text-xs text-neutral-500 font-medium">
                                                        <span>Desktop</span>
                                                        <span>{activeSplitConfig.desktop.value}</span>
                                                    </div>
                                                    <UniversalSlider
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex justify-between items-center text-xs text-neutral-500 font-medium">
                                                        <span>Mobile</span>
                                                        <span>{activeSplitConfig.mobile.value}</span>
                                                    </div>
                                                    <UniversalSlider
    ...
    // Chat Mode Styles: Ride the Rail (NOW USING PROP)
    const getBottomPosition = () => {
        switch (chatMode) {
            case 'full': return 'bottom-[92vh]';
            case 'standard': return 'bottom-[50vh]';
            case 'micro': return 'bottom-[180px]';
            default: return 'bottom-[60px]'; // Nano
        }
    };

    const positionClass = getBottomPosition();

    return (
        <div className={`fixed left-0 right-0 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-xl border-t border-neutral-200 dark:border-neutral-800 z-40 shadow-[-2px_-4px_10px_rgba(0,0,0,0.05)] transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] pb-safe ${positionClass} ${visibilityClass}`}>
            {renderContent()}
        </div>
    );
};
