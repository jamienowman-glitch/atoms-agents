24-Column Layout Grid – Spec for Atoms

Core idea
	•	The entire UI sits on a 24-column fluid grid.
	•	Columns are always equal width, and all spacing (gutters, margins, padding) is controlled by tokens, not hardcoded pixels.
	•	Atoms never guess screen size – they just know how many of the 24 columns they occupy on each breakpoint.

⸻

Columns, Gutters, Margins
	•	Total columns: 24
	•	Unit system is fractions of 24 (1/24, 6/24, 12/24 etc.), not fixed widths.
	•	Horizontal spacing is split into:
	•	Outer margin tokens (left/right page padding).
	•	Gutter tokens (space between columns).
	•	Tokens (examples – names can be adjusted in the registry):
	•	grid_columns = 24
	•	grid_outer_margin_desktop
	•	grid_outer_margin_tablet
	•	grid_outer_margin_mobile
	•	grid_gutter_desktop
	•	grid_gutter_tablet
	•	grid_gutter_mobile

The actual numeric values live in the layout/spacing tokens, not in the atoms.

⸻

Breakpoints

We treat layout as 3 main breakpoints (names, not hardcoded sizes):
	•	viewport = mobile
	•	viewport = tablet
	•	viewport = desktop

For each breakpoint, a component can declare:
	•	span_mobile   → how many of the 24 columns it spans on mobile.
	•	span_tablet   → how many columns on tablet.
	•	span_desktop  → how many columns on desktop.
	•	Optional:
	•	offset_* tokens if we ever want to introduce horizontal offsets.

Atoms know “I am 24/24 on mobile, 12/24 on desktop”, but do not manage media queries; the parent layout/view handles that.

⸻

How atoms use the grid

For every layout-aware atom, we assume:
	•	It exposes layout tokens, e.g.:
	•	grid_span_desktop
	•	grid_span_tablet
	•	grid_span_mobile
	•	It aligns its internal content to this width; any internal padding still comes from spacing tokens, not ad-hoc values.
	•	The view/template layer is responsible for arranging multiple atoms into rows/sections using these 24-column spans.

So:
	•	Atoms = “I occupy N of 24 columns here.”
	•	Views = “Put these atoms into a row/stack using their spans and offsets.”
	•	Global layout = “Define what 24 columns actually look like per breakpoint using tokens.”

⸻

Safe Areas & Full-Bleed
	•	There is a concept of safe area vs full-bleed:
	•	Default: content respects outer margins (grid_outer_margin_*).
	•	Optional: certain atoms/views may declare a full_bleed = true token so they ignore outer margins but still use the 24-column structure internally.
	•	This lets us have hero sections that edge-to-edge while keeping everything on the same mental grid.

⸻

Summary line (for the agent)

Treat the layout as a 24-column token-driven grid. Atoms never hardcode widths; they just expose and consume span_*/layout tokens (fractions of 24). Views/templates decide how atoms sit next to each other, and global layout tokens define gutters and margins per breakpoint.
