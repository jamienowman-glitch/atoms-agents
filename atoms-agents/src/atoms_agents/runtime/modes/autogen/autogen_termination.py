...
def run(input_json: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    try:
        # Atomic runner for mode: autogen.termination
        # Calls framework adapter smoke test
        return adapter.minimal_smoke("autogen.termination", input_json, ctx)
    except SkipMode as e:
        return { "status": "SKIPPED", "reason": str(e), "mode_id": "autogen.termination" }
    except Exception as e:
        return { "status": "FAIL", "mode_id": "autogen.termination", "error": str(e) }
