    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-100 p-8 font-mono">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <section className="bg-zinc-900/50 p-6 border border-zinc-800 rounded-lg col-span-1 lg:col-span-2">
                    <h2 className="text-lg font-bold text-zinc-300 mb-6 flex items-center gap-2">
                        <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
                        Code Generator
                        <span className="text-xs font-normal text-zinc-500 ml-auto bg-zinc-800 px-2 py-0.5 rounded">Marketing Factory</span>
                    </h2>

                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end bg-zinc-950 p-6 rounded border border-zinc-800">
                        {/* Code */}
                        <div className="md:col-span-1">
                            <label className="block text-xs uppercase text-zinc-500 mb-2">Code Name (e.g. REDDIT20)</label>
                            <input
                                id="new-code"
                                className="w-full bg-zinc-900 border border-zinc-800 rounded px-3 py-3 text-sm text-white font-mono uppercase focus:border-emerald-500 outline-none"
                        ...
                        <div className="md:col-span-1">
                            ...
                            <select
                                id="new-type"
                                className="w-full bg-zinc-900 border border-zinc-800 rounded px-3 py-3 text-sm text-zinc-300 focus:border-emerald-500 outline-none appearance-none"
                            >
                        ...
                        {/* Value */}
                        <div className="md:col-span-1">
                            <label className="block text-xs uppercase text-zinc-500 mb-2">Value</label>
                            <input
                                id="new-value"
                                type="number"
                                step="0.01"
                                className="w-full bg-zinc-900 border border-zinc-800 rounded px-3 py-3 text-sm text-white font-mono focus:border-emerald-500 outline-none"
                                placeholder="0.20"
                            />
                        </div>
                        ...
                        <div className="md:col-span-1">
                            <button
                                onClick={async () => {
                                    const codeInput = document.getElementById('new-code') as HTMLInputElement;
                                    ...
                                    if (!codeInput.value || !valueInput.value) return alert("Missing fields");

                                    const { error } = await supabase.rpc('create_discount_code', {
                                        p_tenant_id: tenantId,
                                        p_code: codeInput.value,
                                        p_discount_type: typeInput.value,
                                        p_discount_value: parseFloat(valueInput.value),
                                        p_channel: 'internal'
                                    });

                                    if (error) alert('Error: ' + error.message);
                                    else {
                                        alert('Code Generated: ' + codeInput.value);
                                        codeInput.value = '';
                                        valueInput.value = '';
                                    }
                                }}
                                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded shadow-lg transition-all"
                            >
                                GENERATE CODE
                            </button>
                        </div>
