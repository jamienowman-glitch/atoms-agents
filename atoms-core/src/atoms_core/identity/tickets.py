"""HMAC-signed ticket issuance/validation for Realtime transports."""
...
def _get_secret() -> bytes:
    # Try env first (legacy), then settings system key (atoms-core convention)
    secret = os.getenv("ENGINES_TICKET_SECRET")
    if not secret:
        # Fallback to system key as a shared secret if specific ticket secret missing
        try:
            settings = get_settings()
            secret = settings.SYSTEM_KEY
        except Exception:
            pass
            
    if not secret:
        raise TicketError("TICKET_SECRET or SYSTEM_KEY is required to issue/validate tickets")
...
def issue_ticket(scope: Dict[str, Any], expires_in: int = TICKET_TTL_SECONDS) -> str:
    ...
    Scope must include tenant_id, mode, and project_id.
    """
...
def validate_ticket(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise TicketError("invalid ticket encoding")
