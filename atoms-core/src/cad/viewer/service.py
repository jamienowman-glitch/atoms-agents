...
class StubPlanService:
    def generate_plan(self, *args, **kwargs):
        raise NotImplementedError("Plan service unavailable in standalone mode")

class StubBoQService:
    pass

class StubCostingService:
    def get_cost_model(self, *args, **kwargs):
        raise NotImplementedError("Costing service unavailable in standalone mode")
...
class CadViewerService:
    ...
    def __init__(self, plan_service=None, boq_service=None, costing_service=None):
        self.plan_svc = plan_service or StubPlanService()
        self.boq_svc = boq_service or StubBoQService()
        self.cost_svc = costing_service or StubCostingService()
    ...
    def build_gantt_view(self, project_id: str, cost_model_id: str, context: Optional[Dict[str, Any]] = None) -> CadGanttView:
        ...
        try:
            plan = self.plan_svc.generate_plan(cost_model_id, template_version="1.0.0")
             ...
             # In stub mode, return empty/dummy view or re-raise
             # Re-raising is safer to indicate it's not working yet
             raise
