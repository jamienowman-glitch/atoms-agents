...
## Front-End Handoff (per SKILL instructions, lines 11-65)

The preview/timeline UI must call the new MCPs in the order documented in each muscle's `SKILL.md`. Follow these steps so the front-end consumes the `media_v2` URIs, action tokens, keyframes, and plan metadata that the spec’s cross-cutting coordination section describes:

1. **Render the surface:** POST `/muscle/video/video_planet_surface_renderer/render` with the chosen `surface_params`, `duration_ms`, and optional `frame_rate`. Use the returned `frames` array to drive the preview textures (`media_v2://planet_surface/<slug>/frame_<NNNN>`) and pass the `metadata.atlas_hash` into cache-checking logic. This SKILL (lines 11-34) also explains the lighting/curvature schema that other muscles rely on.
2. **Simulate the run:** Feed the renderer’s output metadata or the desired `surface_params` into `/muscle/video/video_planet_runner/run_plan` along with a `run_duration_ms` (60 seconds by default) and any `speed_profile`. The response contains normals, forwards, and timestamps; keep every `normal` unit-length and pointing toward the planet center (see SKILL notes lines 48-50) before transferring the data to timeline tooling.
3. **Build the preview plan:** Send the `run_metadata` returned by the runner, along with `resolution`, `device_capability`, and an optional `strategy`, to `/muscle/video/video_planet_preview/run_preview`. That endpoint returns `preview_plan.assets` (surface stream + GPU/CPU proxy hints), `preview_plan.timeline` (preview track samples), and `preview_plan.actions`, which already target `video_timeline` and `video_focus_automation`.

When the preview plan contains `preview_plan.actions`, forward them to the existing timeline and focus muscles so the UI automatically reconciles the automation data (per Cross-Cutting Task 2). The action tokens include the `run_metadata.keyframes` payload used for sync, and the `strategy` key informs whether the render path should rely on WebGL textures or CPU fallback proxies. This wiring ensures the UI team follows the spec’s guidance (see `SKILL.md` lines 11-65) while streaming the glowing surface via `media_v2` assets and honoring the GPU/CPU split described in the cross-cutting section.
