...
// Mock context for rescue
const useConsoleContext = () => ({
    runMessages: [
        { id: '1', text: 'Welcome to the wysiwyg builder.', type: 'info', ts: new Date().toISOString(), actor: 'AI', visibility: 'visible' },
        { id: '2', text: 'I am ready to help you build.', type: 'info', ts: new Date().toISOString(), actor: 'AI', visibility: 'visible' }
    ]
});
...
export const MessageStream = () => {
    const { runMessages } = useConsoleContext();
    const visibleMessages = runMessages.filter(message => message.visibility !== 'internal');
