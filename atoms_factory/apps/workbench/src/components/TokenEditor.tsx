...
export const TokenEditor: React.FC<TokenEditorProps> = ({ tokens, onChange, metadata = {}, path = [] }) => {
    if (!tokens || typeof tokens !== 'object') return null;

    // Mobile "Quick Pages" extraction logic
    // We only render this at the ROOT level (path is empty)
    ...
    const renderQuickControl = (fullPath: string, label: string) => {
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <input
                    type="range"
                    min={0} max={100} // Default range, should come from metadata ideally
                    value={Number(val) || 0}
...
const TokenTree: React.FC<{ tokens: any, onChange: (p: string[], v: any) => void, path: string[], metadata: any, isRoot?: boolean }> = ({ tokens, onChange, path, metadata, isRoot }) => {
    ...
    const sortedKeys = [...keys].sort((a, b) => {
        ...
        if (ia !== -1) return -1;
        if (ib !== -1) return 1;
        // Keep special top-levels at top if in GROUPS
