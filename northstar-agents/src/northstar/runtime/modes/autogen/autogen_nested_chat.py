...
def run(input_json: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    try:
        # Atomic runner for mode: autogen.nested_chat
        # Calls framework adapter smoke test
        return adapter.minimal_smoke("autogen.nested_chat", input_json, ctx)
    except SkipMode as e:
        return { "status": "SKIPPED", "reason": str(e), "mode_id": "autogen.nested_chat" }
    except Exception as e:
        return { "status": "FAIL", "mode_id": "autogen.nested_chat", "error": str(e) }
