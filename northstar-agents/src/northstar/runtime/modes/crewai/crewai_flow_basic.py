...
def run(input_json: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    try:
        # Atomic runner for mode: crewai.flow_basic
        # Calls framework adapter smoke test
        return adapter.minimal_smoke("crewai.flow_basic", input_json, ctx)
    except SkipMode as e:
        return { "status": "SKIPPED", "reason": str(e), "mode_id": "crewai.flow_basic" }
    except Exception as e:
        return { "status": "FAIL", "mode_id": "crewai.flow_basic", "error": str(e) }
