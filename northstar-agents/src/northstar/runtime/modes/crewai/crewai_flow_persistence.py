...
def run(input_json: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    try:
        # Atomic runner for mode: crewai.flow_persistence
        # Calls framework adapter smoke test
        return adapter.minimal_smoke("crewai.flow_persistence", input_json, ctx)
    except SkipMode as e:
        return { "status": "SKIPPED", "reason": str(e), "mode_id": "crewai.flow_persistence" }
    except Exception as e:
        return { "status": "FAIL", "mode_id": "crewai.flow_persistence", "error": str(e) }
