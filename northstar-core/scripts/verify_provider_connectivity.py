...
EVIDENCE_DIR = "docs/evidence/bedrock"
...
def phase_0b_discovery(region):
    ...
    try:
        ...
        models_path = f"{EVIDENCE_DIR}/list_foundation_models.{TIMESTAMP}.json"
        ...
        try:
            ...
            profiles_path = f"{EVIDENCE_DIR}/list_inference_profiles.{TIMESTAMP}.json"
        ...
        with open(f"{EVIDENCE_DIR}/model_selection.md", "w") as f:
            f.write(md_content)
        log(f"  Selection report saved to {EVIDENCE_DIR}/model_selection.md")
        
        # Save selection json for resolver
        with open(f"{EVIDENCE_DIR}/latest_selection.json", "w") as f:
...
def phase_0c_live_calls(selected, region):
    ...
    try:
        ...
        with open(f"{EVIDENCE_DIR}/live_text.{TIMESTAMP}.json", "w") as f:
    ...
    try:
        ...
        with open(f"{EVIDENCE_DIR}/live_stream.{TIMESTAMP}.json", "w") as f:
            json.dump(evidence, f, indent=2)
    ...
    if selected["vision"]:
        try:
            ...
            with open(f"{EVIDENCE_DIR}/live_vision.{TIMESTAMP}.json", "w") as f:
                json.dump(evidence, f, indent=2)
...
def phase_1_adk():
    ...
    EVIDENCE_DIR_ADK = "docs/evidence/adk"
