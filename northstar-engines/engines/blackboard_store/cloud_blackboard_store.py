"""Cloud backend implementations for blackboard_store (persistent shared coordination state).
...
class FirestoreBlackboardStore:
    """Firestore-backed persistent blackboard with versioning."""
    
    _collection = "blackboard_store"
    
    def __init__(self, project: Optional[str] = None, client: Optional[object] = None) -> None:
        try:
            from google.cloud import firestore
        except Exception as exc:
            raise RuntimeError("google-cloud-firestore is required for Firestore blackboard") from exc
    ...
    def write(
        ...
        try:
            ...
            logger.error(f"Failed to write blackboard key '{key}': {exc}")
            raise RuntimeError(f"Blackboard write failed: {exc}") from exc
    ...
    def list_keys(
        ...
        try:
            ...
            logger.error(f"Failed to list blackboard keys for run {run_id}: {exc}")
            return []


# ===== DynamoDB Implementation =====

class DynamoDBBlackboardStore:
    """DynamoDB-backed persistent blackboard with versioning."""
    
    def __init__(self, table_name: Optional[str] = None, region: Optional[str] = None) -> None:
        try:
            import boto3
        except Exception as exc:
            raise RuntimeError("boto3 is required for DynamoDB blackboard") from exc
        
        self._table_name = table_name or "blackboard_store"
...
class CosmosBlackboardStore:
    """Cosmos DB-backed persistent blackboard with versioning."""
    
    def __init__(self, endpoint: str, key: str, database: str = "blackboard_store") -> None:
        try:
            from azure.cosmos import CosmosClient
        except Exception as exc:
            raise RuntimeError("azure-cosmos is required for Cosmos blackboard") from exc
