...
from engines.canvas_commands.catalog_service import get_token_catalog_service, TokenCatalogItem
...
@router.get("/{canvas_id}/token_catalog", response_model=List[TokenCatalogItem])
async def get_token_catalog(
    ...
    """GET /canvas/{canvas_id}/token_catalog - Retrieve the merged token catalog.
    
    Returns schemas from Registry merged with instance state from Canvas.
    Optionally scoped to a specific element by `element_id`.
    """
    if auth_context.default_tenant_id != request_context.tenant_id:
        error_response(
            code="tenant_mismatch",
    ...
    try:
        service = get_token_catalog_service(request_context)
        catalog = await service.get_token_catalog(canvas_id, element_id)
        return catalog
    except Exception as e:
        error_response(
            code="token_catalog_error",
            message=str(e),
            status_code=500,
            resource_kind="canvas",
        )
