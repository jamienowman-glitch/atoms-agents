...
async def apply_command(
    ...
    # [NEW] Token Validation for set_token / set_tokens
    if command.type == "set_token":
        path = command.args.get("token_path")
        val = command.args.get("value")
        # Inline import to avoid circular dep if models used in schema
        from engines.tokens.schema import validate_token_value
        if path and not validate_token_value(path, val):
             # We should probably raise 400, but to avoid crashing async workers, 
             # we might need to be careful. However, this is the API layer.
             raise HTTPException(status_code=400, detail=f"Invalid token path or value: {path}={val}")
