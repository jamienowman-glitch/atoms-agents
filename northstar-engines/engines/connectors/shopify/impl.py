...
class ListShopifyFeedsInput(BaseModel):
    pass

class RefreshShopifyFeedInput(BaseModel):
    feed_id: str

class GetShopifyFeedItemsInput(BaseModel):
    feed_id: str
    limit: int = 20
    cursor: Optional[str] = None

async def list_feeds(ctx: RequestContext, input_data: ListShopifyFeedsInput) -> List[Dict[str, Any]]:
    engine = FeedEngine()
    feeds = await engine.list_feeds(ctx)
    return [asdict(f) for f in feeds]

async def refresh_feed(ctx: RequestContext, input_data: RefreshShopifyFeedInput) -> Dict[str, Any]:
    engine = FeedEngine()
    return await engine.refresh_feed(ctx, input_data.feed_id)

async def get_items(ctx: RequestContext, input_data: GetShopifyFeedItemsInput) -> List[Dict[str, Any]]:
    engine = FeedEngine()
