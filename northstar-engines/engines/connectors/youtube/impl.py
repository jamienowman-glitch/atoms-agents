...
class ListYouTubeFeedsInput(BaseModel):
    pass # No params needed, scoped by context

class RefreshYouTubeFeedInput(BaseModel):
    feed_id: str

class GetYouTubeFeedItemsInput(BaseModel):
    feed_id: str
    limit: int = 20
    cursor: Optional[str] = None

async def list_feeds(ctx: RequestContext, input_data: ListYouTubeFeedsInput) -> List[Dict[str, Any]]:
    engine = FeedEngine()
    feeds = await engine.list_feeds(ctx)
    return [{"feed_id": f.feed_id, "name": f.name, "channels": f.channels} for f in feeds]

async def refresh_feed(ctx: RequestContext, input_data: RefreshYouTubeFeedInput) -> Dict[str, Any]:
    engine = FeedEngine()
    return await engine.refresh_feed(ctx, input_data.feed_id)

async def get_items(ctx: RequestContext, input_data: GetYouTubeFeedItemsInput) -> List[Dict[str, Any]]:
    engine = FeedEngine()
    items = await engine.get_items(ctx, input_data.feed_id, input_data.limit, input_data.cursor)
