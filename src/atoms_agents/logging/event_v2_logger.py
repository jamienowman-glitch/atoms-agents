...
- Persist events to Supabase Event Spine V2 (`event_spine_v2_*`)
...
from atoms_core.event_spine.models import EventCreate
from atoms_core.event_spine.repository import EventRepository
from atoms_core.event_spine.service import EventService
...
class RoutingKeys(BaseModel):
    ...
    tenant_id: str = Field(..., description="Tenant ID (UUID string)")
...
class EventV2Logger:
    ...
    def __init__(
        ...
        self._event_spine_service: Optional[EventService] = None

    def _get_event_spine_service(self) -> EventService:
        if self._event_spine_service is not None:
            return self._event_spine_service
        repo = EventRepository(client=self.supabase_client) if self.supabase_client else EventRepository()
        self._event_spine_service = EventService(repo)
        return self._event_spine_service
    ...
    async def emit_event(
        ...
        Args:
            ...
            persist_to_db: Persist to Supabase Event Spine V2 (`event_spine_v2_*`)
        """
        try:
            ...
            # Persist to Supabase (Event Spine V2)
            if persist_to_db and event.persist_policy != PersistPolicy.NEVER:
                try:
    ...
    async def _persist_event(self, event: EventV2) -> None:
        """Persist event to Supabase Event Spine V2 (`event_spine_v2_*`)."""
        ...
        try:
            ...
            # Common during local/dev when EVENT_TENANT_ID isn't wired yet.
            self.logger.debug("Skipping Event Spine persist: tenant_id is not a UUID (%s)", tenant_id)
            return

        run_id = event.run_id or event.correlation_id or event.request_id or event.event_id
        if not run_id:
            run_id = str(uuid.uuid4())

        # Heuristic: canvas-scoped events are whiteboard; everything else defaults to blackboard.
        context_scope = "whiteboard" if event.routing.canvas_id else "blackboard"

        artifacts: List[Dict[str, Any]] = []
        for s in event.media_sidecars or []:
            if not getattr(s, "uri", None):
                continue
            ...
            artifacts.append(
                {
                    ...
                    "uri": s.uri,
                    "metadata": {
                        "object_id": getattr(s, "object_id", None),
                        "artifact_id": getattr(s, "artifact_id", None),
                        "mime_type": getattr(s, "mime_type", None),
                        "size_bytes": getattr(s, "size_bytes", None),
                        "checksum": getattr(s, "checksum", None),
                    },
                }
            )
        ...
        payload["_event_v2"] = {
            ...
            "storage_class": event.storage_class.value,
            "actor_type": event.routing.actor_type.value,
            "routing": event.routing.model_dump(),
        }

        spine_event = EventCreate(
            tenant_id=str(tenant_id),
            mode=os.getenv("EVENT_MODE", "system"),
            ...
            agent_id=event.routing.actor_id,
            sequence_id=event.seq or 0,
            normalized_timestamp=event.ts,
            ...
            client_timestamp=event.ts,
            payload=payload,
            artifacts=artifacts or None,
        )

        svc = self._get_event_spine_service()
        await asyncio.to_thread(svc.append_event, spine_event)
...
async def log_provider_inference(
    ...
    # --- PRIVACY SCRUBBER (LOGGING BOUNDARY) ---
    try:
        from atoms_agents.runtime.privacy.scrubber import PiiScrubber
        scrubber = PiiScrubber()
        
        if input_text:
            scrub_res = scrubber.scrub_text(input_text)
            input_text = scrub_res.cleaned_text
            
        if output_text:
            scrub_res = scrubber.scrub_text(output_text)
            output_text = scrub_res.cleaned_text
            
    except (ImportError, Exception) as e:
        # If scrubbing fails or cannot be loaded during logging, 
        # strictly drop the content to be safe.
        # "Logging persistence paths must not store raw unsanitized model input/output text."
        logger.logger.error(f"Scrubber fail-closed during logging: {e}. Dropping raw content.")
        input_text = "[SCRUBBER_FAILED_DROPPED]"
        output_text = "[SCRUBBER_FAILED_DROPPED]"
