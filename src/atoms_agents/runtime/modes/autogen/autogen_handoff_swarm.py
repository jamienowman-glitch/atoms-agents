...
def run(input_json: Dict[str, Any], ctx: RunContext) -> Dict[str, Any]:
    try:
        # Atomic runner for mode: autogen.handoff_swarm
        # Calls framework adapter smoke test
        return adapter.minimal_smoke("autogen.handoff_swarm", input_json, ctx)
    except SkipMode as e:
        return { "status": "SKIPPED", "reason": str(e), "mode_id": "autogen.handoff_swarm" }
    except Exception as e:
        return { "status": "FAIL", "mode_id": "autogen.handoff_swarm", "error": str(e) }
