...
const initialState: VideoCanvasState = {
...
// --- Reducer ---
function videoCanvasReducer(state: VideoCanvasState, action: Action): VideoCanvasState {
    switch (action.type) {
        case 'SET_PROJECT':
...
// --- Context ---
const VideoCanvasContext = createContext<{
    state: VideoCanvasState;
    dispatch: React.Dispatch<Action>;
} | null>(null);

export const VideoCanvasProvider = ({ children }: { children: ReactNode }) => {
    const [state, dispatch] = useReducer(videoCanvasReducer, initialState);
    return (
        <VideoCanvasContext.Provider value={{ state, dispatch }
        }>
            {children}
...
// --- Hook ---
export const useVideoCanvasStore = () => {
    const context = useContext(VideoCanvasContext);
    if (!context) {
        throw new Error("useVideoCanvasStore must be used within a VideoCanvasProvider");
    }
    return context;
};
